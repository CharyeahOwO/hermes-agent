...
class TestBuildNativeContentParts:
    ...
    def test_empty_text_preserves_only_image_hint(self, tmp_path: Path):
        img = tmp_path / "cat.jpg"
        img.write_bytes(_png_bytes())
        parts, skipped = build_native_content_parts("", [str(img)])
        assert skipped == []
        # Image-only gateway messages should not invent a user question. The
        # path hint is enough context to accompany the native image part.
        assert parts[0]["type"] == "text"
        assert parts[0]["text"] == f"[Image attached at: {img}]"
        assert parts[1]["type"] == "image_url"
...
class TestBuildNativeContentPartsURLs:
    ...
    def test_url_only_preserves_only_image_hint_when_text_empty(self):
        parts, _ = build_native_content_parts(
            "", [], image_urls=["https://example.com/a.png"]
        )
        assert parts[0]["type"] == "text"
        assert parts[0]["text"] == "[Image attached: https://example.com/a.png]"
